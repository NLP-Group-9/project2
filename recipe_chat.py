...
def show_step_by_number(steps, step_num):
    """Display a specific step by number."""
    if 1 <= step_num <= len(steps):
        step = steps[step_num - 1]  # Convert to 0-based index
        print(f"\nStep {step_num}: {step.description}")
    else:
        print(f"\nStep {step_num} doesn't exist. This recipe has {len(steps)} steps.")
...
def handle_how_much_question(ingredients, query):
    """
    ...
    if re.search(r'\b(that|this|it)\b', query):
        print("\nI'm not good with vague references yet. Please mention the ingredient or action by name, like \"how much flour do I need?\" or \"how do I knead the dough?\".")
        return True
...
def handle_what_is_question(query):
    """
...
def handle_how_do_i_question(query):
    """
...
def process_user_query(ingredients, steps, query):
    """
    ...
    if step_num:
        show_step_by_number(steps, step_num)
        return True
    
    # Check for "how much" questions
    if query_lower.startswith("how much"):
        if handle_how_much_question(ingredients, query_lower):
            return True
    
    # Check for "what is" questions
    if query_lower.startswith("what is"):
        if handle_what_is_question(query_lower):
            return True
    
    # Check for "how do i" questions
    if query_lower.startswith("how do i"):
        if handle_how_do_i_question(query_lower):
            return True
...
def main():
    ...
    while True:
        ...
        should_continue = process_user_query(ingredients, steps, query)
